...
{
	uint8_t  sEcIssureAuthCode[6];   // 9F73电子现金发卡行授权码（EC Issuer Authorization Code）
	uint8_t  sTerminalRand[4];       // 9F37 随机数
	uint8_t  sAip[2];                // 82 应用交互特征
	uint8_t  ucIssureAppDatalen;     // 发卡行应用数据 的长度
	uint8_t  sIssureAppData[32];     // 9F10 发卡行应用数据
	uint8_t  sTerminalCapab[3];       // 9F33 终端性能
	uint8_t  sAppAtc[2];             // 9F36应用交易计数器
	uint8_t  sTVR[5];                //95 终端验证结果
	uint8_t  ucTVRoK;
	uint8_t  sTIDSn[8];              // 9F1E接口设备序列号
	uint8_t  ucTIDSnoK;
	uint8_t  sScritptRet[21];        // DF31发卡行脚本结果
	uint8_t  ucScritptRetlen;        // DF31发卡行脚本结果 的长度
	uint8_t  sARQC[8];               // 9F26 授权请求密文
}ST_EMVPRINT_INFO;
...
//    解析保存IC卡55域中打印的IC卡相关数据，为了兼容，不在记录结构里面新增变量，而是使用55域中的数据
void  SavePrintICCData(uint16_t uiTag, uint8_t *psData, int iDataLen);


extern ST_EMVPRINT_INFO gstIccPrintData;   //用于打印的IC卡相关数据
