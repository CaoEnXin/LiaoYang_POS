...
void DispDateTime(void)
{
	...
	//	int   iRet;//, iCnt = 0;
		kbFlush();
		lcdCls();
		lcdDisplay(0, 0, DISP_CFONT|DISP_INVLINE|DISP_MEDIACY, "日期/时间");
       while(1)
       	{
			if( kbhit()==YES )
			{
			...
			if( memcmp(szLastTime, szTime, 6)!=0 )
			{
			   ...
			   lcdDisplay(0, 3, DISP_CFONT|DISP_CLRLINE, "日期:%.4s/%.2s/%.2s",
						szDate, szDate+4, szDate+6);
			   lcdDisplay(0, 6, DISP_CFONT|DISP_CLRLINE, "时间:%.2s:%.2s:%.2s",
						szTime, szTime+2, szTime+4);
...
int DispMenu(const char * menu[], int iLines, int iTitle, int iStartKey, int iEndKey, int iTimeOut)
{
	...
	if( iTitle ) 
	{
		...
		start_line = 2;
		line_per_screen = 3;
		lcdDisplay(0, 0, DISP_CFONT|DISP_REVERSE|DISP_MEDIACY, (char *)menu[0]);
	}
	else 
	{
		start_item = 0;
	...
	while(1) 
	{
		...
		if(key != KEY_TIMEOUT ) 
		{
			stPosParam.OldTime = sysGetTimerCount();
		}
					...
					char* option1, char* option2, uint8_t ucTimeOut)
{
	...
	lcdCls();
	lcdFlip();	
	lcdDisplay(0, 0, DISP_CFONT|DISP_MEDIACY|DISP_REVERSE, szTitle);
	if( iCondition ) 
	{
...
/****************************************************************************
 功能描述:      将以分为单位的int值金额转为以元为单位的asc串表示金额，asc串以'\0'结束
****************************************************************************/
...
/****************************************************************************
 功能描述:      将以分为单位的asc串转为以元为单位的asc串表示金额，asc串以'\0'结束
 ****************************************************************************/
...
/****************************************************************************
 功能描述:     两BCD数字串相加,相加结果放在第一个指针数组里，结果数字串自动以右对齐
****************************************************************************/
...
uint8_t ConvBcdAmount(uint8_t *bcdAmt, uint8_t *amount_ptr)
{
	...
	for(i=0; i<12; i++)
	{
		if( buffer[i]!='0' )
		{
			...
			strcpy((char *)amount_ptr, (char *)buffer+i);
			return amtLen;
		}
	}
