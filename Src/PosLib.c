...
int  PubCheckPosPower(void)
{
	...
	if (iRet==1 || iRet ==0)
	{
		...
		PubDispString("LOWER POWER !", 4|DISP_LINE_CENTER);
		PubWaitKey(5);
		return FALSE;
	}
...
//		lcdFlip();	
	...
	lcdDisplay(0, 0, DISP_CFONT|DISP_REVERSE,"%-16.16s", buffer);
...
int PubGetMenu(uint8_t *pszPrompt, MenuItem *pstMenu, uint8_t ucMode, uint32_t nTimeOut)
{
	...
	// process options
	ucMenuNum     = MIN(ucMenuNum, (uint8_t)99);			// 最多99个菜单项可以被显示
...
//	bBottomPrompt = bUseSno  ? FALSE : (ucMode & MENU_PROMPT);
	...
	while( 1 )
	{
		...
		if( bUseSno || bAutoSno )
		{
			lcdSetIcon(ICON_UP,   (uint8_t)(ucPageNo>0		   ? OPENICON : CLOSEICON));
			lcdSetIcon(ICON_DOWN, (uint8_t)(ucPageNo<ucPageNum-1 ? OPENICON : CLOSEICON));
		}
		else
		{
			lcdSetIcon(ICON_UP,   (uint8_t)(ucSelNo>0		      ? OPENICON : CLOSEICON));
			lcdSetIcon(ICON_DOWN, (uint8_t)(ucSelNo<ucMenuNum-1 ? OPENICON : CLOSEICON));
		}
		...
		for(ucMenuCnt=0,ucLineNo=ucStartLineNo; ucMenuCnt<ucMenuLines;)
		{
			if( pstMenu[ucCurItem].bVisible )
			{
				...
				else
				{
					...
					if( !bUseSno )
					{
						ucRever = (ucCurItem==ucSelNo) ? DISP_REVERSE: 0;
					}
					lcdDisplay(ucCol, ucLineNo, (uint8_t)(ucMenuFont|ucRever), "%.*s",
...
uint8_t PubGetBoolParam(uint8_t ucLine, uint8_t *pszPrompt, uint8_t *pucResult)
{
	...
	if (pszPrompt!=NULL && *pszPrompt!=0)
	{
		...
		lcdFlip();	
		lcdDisplay(0, ucLine, DISP_CFONT, "%.*s", NUM_MAXZHCHARS, (char *)pszPrompt);
	}

	ucFlag = *pucResult;
	while(1)
	{
		PubDisplayMsg(NUM_MAXLINENO-1, (uint8_t *)(ucFlag ? "[YES]" : "[NO]"));
...
int PubFReadN(int fd, void *psData, int iNeedBytes)
{
	...
	psTmp      = (uint8_t *)psData;
	iLeftBytes = iNeedBytes;
	while(iLeftBytes>0)
	{
		iReadBytes = fileRead(fd, psTmp, iLeftBytes);
		if (iReadBytes<=0)
		{
...
int PubFWriteN(int fd, void *psData, int iNeedBytes)
{
	...
	while(iLeftBytes>0)
	{
		iWriteBytes = fileWrite(fd, psTmp, iLeftBytes);
		if (iWriteBytes<=0)
		{
